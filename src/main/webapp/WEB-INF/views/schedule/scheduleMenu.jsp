  ...
  <title>scheduleMenu.jsp</title>
	...
	<script>
		...
		function scheduleInputView() {
			...
			str += '<input type="button" value="취소" onclick="location.reload()" class="btn btn-secondary" />';
			str += '</td></tr>';
			str += '</table>';
			str += '</form>';
			str += '</div>';
		...
		function updateCheck(idx, part, content) {
			...
			str += '<option '+ (part=="모임"? "selected" : "") + '>모임</option>';
			str += '<option '+ (part=="업무"? "selected" : "") + '>업무</option>';
			str += '<option '+ (part=="학습"? "selected" : "") + '>학습</option>';
			str += '<option '+ (part=="여행"? "selected" : "") + '>여행</option>';
			str += '<option '+ (part=="기타"? "selected" : "") + '>기타</option>';
			str += '</select>';
			str += '</td></tr>';
			str += '<tr><th>내용</th><td>';
...
<div class="container">
  <h4>${ymd} 일정입니다.</h4>
  <p>오늘의 일정은 총 <font color="blue"><b>${scheduleCnt}</b></font>건 있습니다.
	<hr class="border">  
		...
		<table class="table table-hover text-center">
			...
			<c:forEach var="vo" items="${vos}" varStatus="st">
				<tr>
					...
					<td>${vo.part}</td>
					<td>
						<input type="button" value="수정" onclick="updateCheck('${vo.idx}','${vo.part}','${fn:replace(vo.content,newLine,'<br/>')}')" id="scheduleUpdateOpen${vo.idx}" class="scheduleUpdateOpen btn btn-warning btn-sm" />
